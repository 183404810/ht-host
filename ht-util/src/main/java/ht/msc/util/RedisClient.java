...
public class RedisClient {
	...
	public boolean checkKeyExisted(Object key) {
		...
		try {
			if ((key instanceof String) && conn.exists((String) key)) {
				// 字符串key存在，直接返回
				returnConnection(conn);
				return true;
			}
	...
	public long delete(String key) {
		...
		try {
			...
			if (result == 0) {
				result = conn.del(serializable(key).toString());
			}
			if(logger.isDebugEnabled()){
				logger.debug("delete rediscatch key:"+key);
			}
			returnConnection(conn);
			return result;
		} catch (Exception e) {
	...
	public long delete(byte[] key) {
		...
		try {
			long result = conn.del(key.toString());
			returnConnection(conn);
			return result;
		} catch (Exception e) {
			logger.error("",e);
	...
	public Object get(Object key) {
		...
		if (data != null) {
			if(logger.isDebugEnabled()){
				logger.debug("get rediscatch key:"+key+" usedtime:"+(System.currentTimeMillis()-starttime));
			}
