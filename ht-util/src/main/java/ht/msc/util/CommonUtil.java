...
//			logger.error("",e);
	...
	public static void main(String[] agrs){
		...
		Integer singleResult =new Integer(0);
		String hh=String.valueOf(singleResult.intValue()).equals("0")?
				"合格":String.valueOf(singleResult.intValue()).equals("1")?"不合格":"/";
		System.out.println(hh);
		
		SystemUser s=new SystemUser();
		s.setAddressIp("ddd");
		s.setCreateTime(new Date());
//		s.setPwdModifyTime(new Date());
		s.setModifyTime(new Date());
		SystemUser s1=new SystemUser();
		
		try{
			setNullToDefaultValue();
...
//		}
		...
		List<Map<String,Object>> tableA = new ArrayList<Map<String,Object>>();
		...
		List<Map<String,Object>> tableB = new ArrayList<Map<String,Object>>();
		...
		String json = multiTablesToJson(map);
		System.out.println("json==="+json);
		
		parseJsonToMultiTables(json);
		
	}
	
	/**
	 * 将多表数据封装成json字符串
	 * @param map 表名为key, value为表数据
	 * @return
	 */
	public static String multiTablesToJson(Map<String,List<Map<String,Object>>> map){
		List<TableEntity> entityList = new ArrayList<TableEntity>();
		if(map != null && !map.isEmpty()){
			Set<Entry<String, List<Map<String, Object>>>> set = map.entrySet();
			Iterator<Entry<String, List<Map<String, Object>>>> iterator = set.iterator();
