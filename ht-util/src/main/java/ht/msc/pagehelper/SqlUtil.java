...
import ht.msc.util.DataPrivilegeUtils;
import ht.msc.util.LogQuerySqlUtils;
import ht.msc.util.ThreadLocals;
...
public class SqlUtil {
    ...
    private static final String SUFFIX_PRIVILEGE="_Privilege";
    //第一个分页参数
    ...
    public Object processPage(Invocation invocation) throws Throwable {
    	try{
    		...
    		clearLocalPage();
    	    LogQuerySqlUtils.clearTreadLocal();
    	}
    }
    ...
    private Object _processPage(Invocation invocation) throws Throwable {
        ...
        if (SqlUtil.getLocalPage() == null && rowBounds == RowBounds.DEFAULT) {
        	...
        	processPrivilegeMappedStatement(ms, sqlSource, args);
    ...
    * @param args
    */
   public void processPrivilegeMappedStatement(MappedStatement ms, SqlSource sqlSource, Object[] args) {
       args[0] = getMappedStatement(ms, sqlSource, args[1], SUFFIX_PRIVILEGE);
    ...
    private static class Db2Parser extends SimpleParser {
        ...
        public String getPageSql(String sql) {
        	...
        	if(tempSql.contains("ORDER")){
        		 int index =tempSql.lastIndexOf("ORDER");
        			 if(index>0){
        				 orderBy = tempSql.substring(index);  
        			 }
        		 ...
        		 if(StringUtils.isNotEmpty(orderBy)){
        			 ...
        			 for(int i=0;i<orderBys.length;i++){
        				 String temp =orderBys[i];
        				 if(i==orderBys.length-1){
        					 sb.append(temp.substring(temp.indexOf(".")+1)); 
        				 }else{
        					 sb.append(temp.substring(temp.indexOf(".")+1)+",");
        				 }
        			 }
        			 if(sb.toString().contains("ORDER")){
        				 ...
        				 returnStr ="ORDER BY "+ sb.toString();
        			 }
        		 }
        	}
        	
            StringBuilder sqlBuilder = new StringBuilder(sql.length() + 120);
            sqlBuilder.append("select * from (select tmp_page.*,rownumber() over("+returnStr+") as row_id from ( ");
            sqlBuilder.append(sql);
            sqlBuilder.append(" ) as tmp_page)tmp_db2_page where row_id between  ? and ?");
            return sqlBuilder.toString();
        }
    ...
    private class MyDynamicSqlSource implements SqlSource {
         ...
         * 只做权限过滤
         */
        private Boolean isOnlyPrivilege;

        public MyDynamicSqlSource(Configuration configuration, SqlNode rootSqlNode, Boolean count,Boolean isOnlyPrivilege) {
            this.configuration = configuration;
            this.rootSqlNode = rootSqlNode;
            this.count = count;
            this.isOnlyPrivilege=isOnlyPrivilege;
        }
        ...
        public BoundSql getBoundSql(Object parameterObject) {
        	...
        	if(isOnlyPrivilege){
        		...
        		sqlSource=getprivilegeSqlSource(configuration, sqlSource, parameterObject);
        	}else{
        		sql=LogQuerySqlUtils.getlogSql(sql);
		        if (count) {
		        	sqlSource = sqlSourceParser.parse(sql, parameterType, context.getBindings());
    ...
    private class MyProviderSqlSource implements SqlSource {
        ...
        private MyProviderSqlSource(Configuration configuration, ProviderSqlSource providerSqlSource, Boolean count, Boolean isOnlyPrivilege) {
            this.configuration = configuration;
        ...
        public BoundSql getBoundSql(Object parameterObject) {
            ...
            if(isOnlyPrivilege){
            	 return new BoundSql(
	                        configuration,
	                        DataPrivilegeUtils.handlerDataPrivilege(boundSql.getSql()),
    ...
    private MappedStatement getMappedStatement(MappedStatement ms, SqlSource sqlSource, Object parameterObject, String suffix) {
        ...
        boolean islog= StringUtils.isNotBlank(ThreadLocals.getLogMasterModule())?true:false;
        if (parser.isSupportedMappedStatementCache() ) {
            try {
            	if(StringUtils.isNotBlank(ThreadLocals.getLogMasterModule()) && !ms.getId().endsWith("_log")){
            		 qs = ms.getConfiguration().getMappedStatement(ms.getId() + suffix+"_log");
            	}else if(ms.getId().endsWith("_log")){
            		qs = ms.getConfiguration().getMappedStatement(ms.getId());
            	}else{
        ...
        if (qs == null) {
            ...
            if (parser.isSupportedMappedStatementCache()) {
                try {
                    if(islog){
                    	if(!ms.getConfiguration().getMappedStatementNames().contains(id + suffix+"_log")){
                      		ms.getConfiguration().addMappedStatement(qs);
                      	}
               		 	qs= ms.getConfiguration().getMappedStatement(id + suffix+"_log");
                    }
                } catch (Exception e) {
                    //ignore
                }
    ...
    private MappedStatement newMappedStatement(MappedStatement ms, SqlSource sqlSource, String suffix) {
        ...
        if(StringUtils.isNotBlank(ThreadLocals.getLogMasterModule()) && !ms.getId().endsWith("_log"))id=id+"_log";
        ...
        if(suffix==SUFFIX_COUNT){
        	...
        	LogQuerySqlUtils.changeResult2Log(builder, ms, id);
    ...
    private SqlSource getsqlSource(MappedStatement ms, SqlSource sqlSource, Object parameterObject, String suffix) {
        ...
        if (isDynamic(ms)) {
            ...
            return new MyDynamicSqlSource(ms.getConfiguration(), mixedSqlNode, suffix == SUFFIX_COUNT,suffix==SUFFIX_PRIVILEGE);
        } else if (sqlSource instanceof ProviderSqlSource) {
            return new MyProviderSqlSource(ms.getConfiguration(), (ProviderSqlSource) sqlSource, suffix == SUFFIX_COUNT,suffix==SUFFIX_PRIVILEGE);
        }
        //如果是静态分页sql
        ...
        else if(suffix==SUFFIX_PRIVILEGE){
        	return getprivilegeSqlSource(ms.getConfiguration(), sqlSource, parameterObject);
        }
        //如果是静态count-sql
        else {
    ...
    private SqlSource getPageSqlSource(Configuration configuration, SqlSource sqlSource, Object parameterObject) {
        BoundSql boundSql = sqlSource.getBoundSql(parameterObject);
        return new StaticSqlSource(configuration, parser.getPageSql(DataPrivilegeUtils.handlerDataPrivilege(boundSql.getSql())), parser.getPageParameterMapping(configuration, boundSql));
    }
    ...
    private SqlSource getCountSqlSource(Configuration configuration, SqlSource sqlSource, Object parameterObject) {
        BoundSql boundSql = sqlSource.getBoundSql(parameterObject);
        return new StaticSqlSource(configuration, parser.getCountSql(DataPrivilegeUtils.handlerDataPrivilege(boundSql.getSql())), boundSql.getParameterMappings());
    }

    private SqlSource getprivilegeSqlSource(Configuration configuration, SqlSource sqlSource, Object parameterObject){
    	 BoundSql boundSql = sqlSource.getBoundSql(parameterObject);
         return new StaticSqlSource(configuration,  DataPrivilegeUtils.handlerDataPrivilege(boundSql.getSql()), boundSql.getParameterMappings());
    }
