...
public class YouGouJSMVCGeneratorConfiguration extends TypedPropertyHolder
{
  ...
  public String getExtendSupClassDoMain() {
    ...
    if ((this.extendSupClass != null) && (this.extendSupClass.length() > point))
      this.extendSupClassDoMain = this.extendSupClass.substring(point + 1);
    else {
  ...
  public XmlElement toXmlElement() {
    XmlElement answer = new XmlElement("jsMVCGenerator");
    ...
    if (this.extendSupClass != null) {
      answer.addAttribute(new Attribute("extendSupClass", this.extendSupClass));
    }

    if (this.enableSupClassGenericity != null) {
      answer.addAttribute(new Attribute("enableSupClassGenericity", this.enableSupClassGenericity));
    }
    if (this.interfaceExtendSupInterface != null) {
      answer.addAttribute(new Attribute("interfaceExtendSupInterface", this.interfaceExtendSupInterface));
    }
    if (this.enableInterfaceSupInterfaceGenericity != null) {
      answer.addAttribute(new Attribute("enableInterfaceSupInterfaceGenericity", this.enableInterfaceSupInterfaceGenericity));
    }
